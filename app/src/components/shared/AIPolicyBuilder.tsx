...
import { Zap, Bot, ArrowRight, ShieldCheck, CheckCircle2 } from "lucide-react";
import { useStore } from "@/store";


export function AIPolicyBuilder({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const [prompt, setPrompt] = useState("");
  const [status, setStatus] = useState<"idle" | "generating" | "review" | "deployed">("idle");
  ...
  const handleDeploy = () => {
    // Upsert a demo policy based on the prompt
    upsertPolicy({
    ...
    setTimeout(() => {
      onOpenChange(false);
      setTimeout(() => setStatus("idle"), 500); // reset state after close
    }, 2000);
  };
          ...
          {status === "idle" && (
            <div className="space-y-4">
              ...
              <div className="flex justify-end">
                <Button data-tour="ai-policy-generate" onClick={handleGenerate} disabled={!prompt} className="gap-2">
          ...
          {status === "review" && (
            <div className="space-y-6 animate-in fade-in zoom-in duration-300">
              ...
              <div className="flex justify-between items-center">
                ...
                <Button data-tour="ai-policy-deploy" onClick={handleDeploy} className="gap-2 bg-primary text-primary-foreground">
                  Deploy to Safes <ArrowRight className="h-4 w-4" />
                </Button>
              </div>
            </div>
          )}
          ...
          {status === "deployed" && (
            <div className="py-12 flex flex-col items-center justify-center space-y-4 animate-in fade-in slide-in-from-bottom-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mt-1">Autonomous execution is now active.</p>
              </div>
            </div>
          )}
        </div>
