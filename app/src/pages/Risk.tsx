...
export function Risk() {
  ...
  return (
    <div id="risk-section" data-tour="risk-section" className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {counterparties.map(c => {
            const isNewVendor = c.name === "Northwind Logistics";
            const riskBadge = isNewVendor 
              ? <Badge variant="outline" className="bg-chart-3/10 text-chart-3 border-chart-3/20 text-[10px]">Elevated Risk</Badge>
              : <Badge variant="outline" className="bg-chart-5/10 text-chart-5 border-chart-5/20 text-[10px]">Low Risk</Badge>;
            const rationale = isNewVendor
              ? "First-time vendor detected. No historical transaction volume on Base Sepolia. AI recommends dynamic approval threshold reduction pending successful first settlement."
            ...
            const volume = isNewVendor ? "$0.00" : "$124.5k";
            const identity = isNewVendor ? "0x89A...1b34" : "0x71C...4e21";

            return (
            <Card key={c.id} data-tour={isNewVendor ? "risk-new-counterparty" : undefined} className="group hover:border-primary/50 transition-all overflow-hidden">
              <CardHeader className="pb-2 border-b border-white/5 bg-muted/20">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {isNewVendor && <Badge variant="secondary" className="text-[9px] uppercase tracking-tighter">New</Badge>}
                  </div>
                  {riskBadge}
                </div>
              </CardHeader>
              <CardContent className="pt-4 space-y-4">
                <div className="flex items-center gap-4">
                  <div className="flex-1 space-y-1">
                    ...
                    <p className="text-[11px] font-mono truncate text-foreground/80">{identity}</p>
                  </div>
                  <div className="text-right space-y-1">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground tracking-tighter">Volume (30d)</p>
                    <p className="text-[11px] font-bold">{volume}</p>
                  </div>
                </div>
                
                <div data-tour={isNewVendor ? "risk-new-rationale" : undefined} className="rounded-lg bg-black/20 p-3 border border-white/5">
